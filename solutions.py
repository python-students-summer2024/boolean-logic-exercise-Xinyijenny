...
def is_sweltering():
    ...
    temp = input("what's the tempreture in Farenheit(e.g. 35F)?")
    
    if temp.isnumeric():#check if user only type in number
        temp=int(temp)
    
    else: #if user follow the input example
        temp = int(temp[:len(temp)-1]) # slice the units and convert to int
...
def is_warm():
    ...
    """
    # write your code for this function below this line.
    temp = input("\nLets check if it is a warm day, \nwhat's the tempreture in Farenheit(e.g. 35F)?")
...
def is_humid():
    ...
    """
    # write your code for this function below this line.
    humid = input("Is it humid now? Please answer Yes or No.")
    # limit the answer to 3 character, prevent if they answer "yes it is" 
    humid = humid[:3].lower() # make sure the answer is all lowercased
...
def is_inclement():
    ...
    """
    # write your code for this function below this line.
    wea = input("What is the weather today?")
    wea = wea.lower()
    if "rain" in wea or "snow" in wea or "sleet" in wea :# find "rain", "snow", "sleet" in the user's respond
        return True
    else:
        return False
